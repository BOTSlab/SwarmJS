...
const getDataSetsCount = (data): number => {
  return Object.values(data).reduce<number>((count: number, dataSets:{length: number}) => count + dataSets.length, 0);
}
  

const getDomains = (data): { xMinMax: [number, number], yMinMax: [number, number]} => {
  ...
  const xMinMax: [number, number] = [+timeIntervals[0], +timeIntervals[timeIntervals.length - 1]];
  const yMinMax: [number, number] = d3.extent(
    Object.values(data)
      .reduce<number[]>((acc, cur: number[]) => ([...acc, ...cur]), [])
...
const addXAxis = (
  svgElem, xScale, { actualWidth, actualHeight, margin, xTitle }: any = {}
) => {
  svgElem.append('g')
    .classed('x-axis', true)
...
const addYAxis = (
  svgElem, yScale, { actualHeight, margin, yTitle }: any = {}
) => {
  svgElem.append('g')
    .classed('y-axis', true)
...
export const updateScales = (svgElem, data, graphSettings, curScales) => {
  ...
  const ret: {xScale?:d3.ScaleLinear<number, number, never>, yScale?: d3.ScaleLinear<number, number, never>} = {};

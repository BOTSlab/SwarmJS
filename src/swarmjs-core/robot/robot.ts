...
export default class Robot {
  ...
  constructor(
    ...
    World.add(this.scene.world, this.body); // TODO: This should be done by the thing creating the robot, not the robot
    Body.setAngularVelocity(this.body, 1);
    this.scene.engine.velocityIterations = 10;
    this.scene.engine.positionIterations = 10;
  ...
  timeStep() {
    ...
    let actuate;
    try {
      actuate = getController(this, this.controller.actuators)
    } catch (error) {
      // console.log('Error in actuator controller:', error);
    }
    // Actuate
    if (actuate && typeof actuate === 'function') {
      actuate(this.sensors, this.actuatorManager.actuators);
    }
  }

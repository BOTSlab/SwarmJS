    ...
    rends.call(d3.drag()
      .on('start', (event, d: any) => {
        ...
        if (dragDef?.onStart?.log) {
          const t = dragDef.onStart.log
            .map((logginProp) => ([logginProp.prop, parseAttr(d, logginProp, scene)]))
      ...
      .on('drag', (event, d: any) => {
        if (dragDef?.onDrag?.log) {
          console.log(`${d.id}: `, ...dragDef.onDrag.log.map((prop) => parseAttr(d, prop, scene)));
        }
      ...
      .on('end', (event, d: any) => {
        ...
        if (dragDef?.onEnd?.log) {
          console.log(`${d.id}: `, ...dragDef.onEnd.log.map((prop) => parseAttr(d, prop, scene)));
        }
      ...
      const getPoints = (d) => {
        if (Array.isArray(def.dynamicAttrs.points)) {
          return def.dynamicAttrs.points
            .map((attr) => {
              const parsedPoint = parseAttr(d, attr, scene);
              return {
                ...parsedPoint
              };
            });
        ...
        if (def.dynamicAttrs.points.prop) {
          const pointList = parseAttr(d, def.dynamicAttrs.points, scene);
